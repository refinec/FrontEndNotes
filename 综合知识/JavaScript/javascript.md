...
### `prototype`和`__proto__`这两个属性有何区别？

> **`prototype` 只有**“(构造)函数”**才有；**
>
> `__proto__`**（规范写法是 `[[Prototype]]`，通过 `Object.getPrototypeOf` 访问）任何对象都有，它指向自己的“原型对象”**
>
> **函数有 `prototype`，实例有`__proto__`；`prototype` 用于放公共方法，`__proto__` 用于链式查找。**

* 所属主体不同

  | 属性        | 存在于谁身上                   | 值是什么                                         |
  | ----------- | ------------------------------ | ------------------------------------------------ |
  | `prototype` | **函数对象**（特别是构造函数） | **一个对象**，将来被当成实例的 \`\[\[Prototype]] |
  | `__proto__` | **所有普通对象**               | **一个对象**，即该对象的“原型”                   |

* 用途不同

  * **`Fn.prototype`**
    用于**共享属性/方法给通过 `new Fn()` 创建出的实例**。

    ```js
    function Foo() {}
    Foo.prototype.speak = () => console.log('hi');
    
    const f = new Foo();
    f.speak(); // hi （沿着 f.__proto__ 找到 Foo.prototype）
    ```

  * **`obj.__proto__`**（或 `Object.getPrototypeOf(obj)`）
    用于**读取/设置对象的原型链**，实现继承链查找。
...
> **`prototype` 和 `__proto__` 职责完全不同，如果强行“统一”成同一个属性，会把「对象的私有原型链」与「构造函数的共享模板」混为一谈，破坏 JavaScript 继承模型的简洁性与内存效率。**

* 两个属性本质上的“身份”不同

| 属性        | 所属对象     | 存什么                 | 被谁使用                                           |
| ----------- | ------------ | ---------------------- | -------------------------------------------------- |
| `prototype` | **函数对象** | **一个对象模板**       | `new` 运算时，把此模板复制给实例的 `[[Prototype]]` |
| `__proto__` | **所有对象** | **指向当前对象的原型** | 运行期属性查找链                                   |

- 前者是 **“工具箱”**（构造函数把自己的工具箱给别人用）。后者是 **“导航仪”**（每个对象拿着导航仪去找父级）。
  把导航仪和工具箱合并，就会让“每个实例都背着一个巨大的工具箱”，既浪费内存又难以维护。
- **`prototype` 是“构造函数的公共模具”，`__proto__` 是“实例的私有路线”；**
  **两者分工不同，合并会破坏内存共享与继承链，因此无法也不应该统一。**
