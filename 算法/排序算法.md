...
function insertionSort(arr) {
    for (let i = 1; i < arr.length; i++) { // 循环arr.length - 1次
        let preIndex = i - 1;
        let cur = arr[i]; // 保存当前值
        while (0 <= preIndex && cur < arr[preIndex]) {
            arr[preIndex + 1] = arr[preIndex]; // 大的元素排到后面一位
            preIndex--;
        }
        arr[preIndex + 1] = cur
    }
...
function selectSort(arr) {
  let len = arr.length;
  let minNumIndex = 0; // 待排序数组中最小的元素下标
